...
import { setSupabaseToken } from './lib/_supabase-token'
...
;(() => {
  ...
  fetch(`${base}/api/app/${projectId}/session`, {
    ...
    body: JSON.stringify({ isStandalone, isMobile, isIOS }),
  }).catch(() => {})
})()
...
function WhackaBadge() {
  // Per-device dismiss (the ✕). Kept for everyone.
  const [dismissed, setDismissed] = React.useState(() => {
    try { return localStorage.getItem('_whacka_badge_hidden') === '1' } catch { return false }
  })
  // Pro/Ultra owners can remove the badge for ALL visitors. Seed from a cached
  // answer so repeat loads don't flicker, then confirm with the server.
  const [proHidden, setProHidden] = React.useState(() => {
    try { return localStorage.getItem('_whacka_badge_pro_hidden') === '1' } catch { return false }
  })
  ...
  React.useEffect(() => {
    ...
    if (!projectId || import.meta.env.VITE_PREVIEW_TOKEN) return
    const base = import.meta.env.VITE_API_BASE || ''
    fetch(`${base}/api/app/${projectId}/badge`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (!d) return
        const hide = !!d.hideBadge
        setProHidden(hide)
        try { localStorage.setItem('_whacka_badge_pro_hidden', hide ? '1' : '0') } catch {}
      })
      .catch(() => {})
  }, [])

  const visible = !dismissed && !proHidden
  ...
  return (
    <a
      href="https://whacka.app/?utm_source=whacka_badge"
      target="_blank"
...
// ── Google OAuth return ──────────────────────────────────────────────
// /auth/app-callback redirects back here with a single-use #wk_code. Exchange
// it for the session bundle, persist it, then reload so auth.js / getAppUserId()
// pick up the signed-in identity on a clean boot. The fragment is stripped
// before the network call so the code never lingers in history.
;(function handleOAuthReturn() {
  let code = null
  try {
    const m = location.hash.match(/[#&]wk_code=([^&]+)/)
    if (m) {
  ...
  fetch(`${API_BASE}/api/auth/exchange-code`, {
    ...
    .then(({ token, supabaseToken, user, migration }) => {
      try {
        localStorage.setItem('__whacka_auth_token', token)
        localStorage.setItem('__whacka_auth_user', JSON.stringify(user))
        if (migration && !migration.migrated) {
          const anonId = localStorage.getItem('__whacka_user_id')
    ...
    .catch((err) => {
      console.error('[auth] Google sign-in exchange failed:', err)
      mountApp()
    })
})()

// Dismiss the inline cold-boot splash (injected into published HTML at build
// time) once the app has mounted and painted its first frame. No-op in the
// editor preview, which has no #app-splash.
;(() => {
  ...
  // Double rAF: let React commit, then let the browser paint the app's first
  // frame, before fading the splash out — avoids a flash of empty #root.
